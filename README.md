...
# 1_CRUK.sh script
## Requirements
  * Requires variables files generated by MakeVariablesFiles.jar (https://github.com/AWGL/MakeVariableFiles/blob/master/Main.java).
...
### Caveats
BSCLIv2 requires that samples are 'globally unique' - previously it was sufficient for the projectID to be unique. As a results, prior to uploading
sample FASTQ files to basespace, samples files are renamed to include projectId; i.e. $projectId-$sampleId_S7_L001.fastq. 

The project name is entered in the Experiment Name field of the Illumina SampleSheet.csv. 
...
## Introduction
This script downloads the basespace analysis for each sample pair stipulated in the 'appsessions.txt' file. This file is generated by 2_CRUK when the app
is started. 
